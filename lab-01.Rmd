...
# n_2021 and n_2022 are integers, admission, setting and region are characters
...
```{r Ex_f}
visitors %>% filter(50000<n_2022, n_2022<100000) %>% count()
```
...
```{r Ex_g}
visitors %>% count(region) %>% filter(n>50)
```
...
**not applicable, because some data in the 2021 admissions is missing**
...
``` {r Ex 7}
visitors %>% count(setting) %>% arrange(desc(n))
```
...
**The data is positively skewed so there are a few attractions with a much greater number of visitors acting as outliers**
...
```{r Ex_m}
visitors_by_free <- visitors_with_nations %>% group_by(nation) %>% filter(admission == "Free") 

visitors_by_free

count(visitors_by_free)

visitors_totals <- visitors_with_nations %>% group_by(nation) %>% count(nation)

visitors_totals

proportions_by_nations <- visitors_by_free/visitors_totals

proportions_by_nations
